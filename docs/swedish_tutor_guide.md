...
```powershell
python -u perception\download_openface_weights.py --out-dir weights
```
...
python -u integration\openai_swedish_tutor.py `
  ...
  --display --wait-speak-end --use-asr --listen-timeout 8 `
  --use-openai --debug
```
...
If you ask a question (for example: "why do we say ut here?"), it answers the doubt and then asks whether to continue or repeat.
